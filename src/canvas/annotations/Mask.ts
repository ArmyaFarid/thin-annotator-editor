...
import {FreeformPath} from "@/canvas/annotations/FreeformPath.ts";
...
export class Mask implements AnnotationObject {
    ...
    render(ctx: CanvasRenderingContext2D, state: RenderState, scale: Scale): void {
        ...
        for (const layer of this.layers) {
            if (layer.rleMask) {
                ...
                this.makeShapeObject(layer.canvasShape).render(ctx, state, scale);
            }
        }
        ctx.restore();
    }
    ...
    hitTest(x: number, y: number): boolean {
        for (const layer of this.layers) {
            if (layer.rleMask) {
                ...
                if (this.makeShapeObject(layer.canvasShape).hitTest(x, y)) return true;
            }
        }
        return false;
    }
    ...
    getBounds(): Rect {
        ...
        for (const layer of this.layers) {
            ...
            if (layer.rleMask) {
                ...
                b = this.makeShapeObject(layer.canvasShape).getBounds();
            } else {
                continue;
            }
    ...
    private makeShapeObject(s: MaskLayer["canvasShape"]): FreeformPath | Polygon {
        if (!s) throw new Error("no shape");
        if (s.kind === "freeform") return new FreeformPath(s.id, s.points, s.color, s.strokeWidth);
        return new Polygon(s.id, s.vertices, s.fillColor, s.strokeColor);
    }
