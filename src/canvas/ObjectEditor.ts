...
export class ObjectEditor {
    ...
    private delPos(layer: MaskLayer, scale: Scale): {x: number; y: number} | null {
        if (layer.canvasShape?.kind === "polygon") {
            ...
            // Place button just outside the top-right of the polygon's bounding box
            return {x: maxX + DEL_R + 4, y: minY};
        }
        if (layer.canvasShape?.kind === "freeform") {
            const pts = layer.canvasShape.points;
            const maxX = Math.max(...pts.map(pt => pt.x)) * scale.x;
            const minY = Math.min(...pts.map(pt => pt.y)) * scale.y;
            return {x: maxX + DEL_R + 4, y: minY};
        }
        if (layer.rleMask) {
